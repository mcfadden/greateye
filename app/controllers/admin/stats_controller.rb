class Admin::StatsController < Admin::BaseController
  def index
    ...
    first_event_timestamp = CameraEvent.ordered.unkept.where.not(event_timestamp: nil).first.event_timestamp || 30.days.ago
    @grouped_events = CameraEvent.group_by_day(:event_timestamp, range: (first_event_timestamp..Time.now), format: "%m-%d").count
